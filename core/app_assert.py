...
class DeepLoggingAssert:
    ...
    # 断言项目树下不存在子节点
    def assert_tree_node_no_exists(self, parent_node_name, child_node_name):
        """
        断言树节点下存在指定子节点
        :param parent_node_name: 父节点名称（默认Demo）
        :param child_node_name: 子节点名称（默认data_test.xtf）
        ...
        assert not child_node.exists(timeout=CONTROL_TIMEOUT), \
            f"❌ [{parent_node_name}]节点下不应该存在[{child_node_name}]子节点，但它出现了！"
        print(f"✅ [{parent_node_name}]节点下不存在[{child_node_name}]子节点")